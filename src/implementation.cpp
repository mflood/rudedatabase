...
char *Implementation::escape(const char *string, int length)
{
	...
	mysql_escape_string(retval,string,length);
	return retval;
}
...
RUDEBOOL Implementation::connect()
{
	if(!d_connected)
	{
		MYSQL *initconn = mysql_init(0);
		if(initconn)
		{
			MYSQL * conn = mysql_real_connect(initconn, 
						d_server.c_str() , 
						d_username.c_str(), 
						d_password.c_str(), 
						d_database.c_str(), 
...
RUDEBOOL Implementation::executeQuery(const char *query)
{
	if(query)
	{
		if(d_connected)
		{
			if( mysql_query(d_conn, query) == 0)
			{
				return true;
			}
			else
			{
				setError(mysql_error(d_conn));
			}
		}
		else
		{
	...
	if(query)
	{
		if(d_connected)
		{
			if( mysql_query(d_conn, query) == 0)
			{
				//get result set
				//
				MYSQL_RES *result = mysql_store_result(d_conn);
				if(result)
				{
					//get first row
					//
...
RUDEBOOL Implementation::storeResultQuery(const char *query)
{
	if(d_connected)
	{
		if(query)
		{
			...
			{
				//get result set
				d_result = mysql_store_result(d_conn);
				if(d_result)
				{
					return true;
				}
...
RUDEBOOL Implementation::useResultQuery(const char *query)
{
	if(d_connected)
	{
		if(query)
		{
			if( mysql_query(d_conn, query) == 0)
			{
				d_result = mysql_use_result(d_conn);
				if(d_result)
				{
					return true;
				}
